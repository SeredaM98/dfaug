...
class DMTEVT_Encoder(nn.Module):
    ...
    def InitNetworkMLP(
        ...
        #     #     Transformer_layer,P*Log(Q) - (1-P)*log(1-Q) что это за функция потерь?
...
class DMTEVT_model(LightningModule):
    ...
    def LossManifold(
        ...
        # data_2 = input_data[batch_size:] # фичи аугментаций
        # dis_P_2 = self._DistanceSquared(data_1, data_2)
...
"""
    """def forward_simi(self, x, nu=1, tau=1):
        ...
        # for i in range(30):
        # print([(data_pre==i).sum() for i in rang(30)])
        # import pdb; pdb.set_trace()
        # self.aim_cluster = 0
        # center = self.Kcluster_layer.weight.data.detach()
        # center_vis = self.vis(center)
...
"""
    ...
    def training_step(self, batch, batch_idx):
        ...
        #            plt.imshow(data_at[i].cpu().detach().numpy().reshape(28, 28))
         ...
         #       else:
         #           plt.imshow(data_aat[i-8].cpu().detach().numpy().reshape(28, 28))
         #           plt.colorbar(shrink=0.83)
    ...
    def validation_step(self, batch, batch_idx, test=False):
        ...
        if self.em_state == "s2" or self.em_state == "m":
            self.vis.eval()
            self.enc.eval()
            cond, _ = self(data_at, batch_idx)
            with torch.no_grad():
    ...
    def on_validation_epoch_end(self):
        ...
        if "Mnist" in self.hparams.data_name:
            ...
            # image_array = self.samples[0].detach().cpu().numpy().reshape(28, 28)
            # print("imag? =", image_array)
